...
After having everything installed, use the following command to run the split central:
...
cargo run --release --bin central
    ...
    Finished `release` profile [optimized + debuginfo] target(s) in 11.70s
     Running `espflash flash --monitor --port /dev/cu.usbmodem211401 target/riscv32imac-unknown-none-elf/release/central`
...
# Check all segments
espsegs target/riscv32imac-unknown-none-elf/release/central --chip esp32h2
```