...
async fn main(_spawner: Spawner) {
    ...
    info!("ee config: {}, {}, {}", eeprom_storage_config.page_size, eeprom_storage_config.storage_size, eeprom_storage_config.start_addr);
    // Use internal flash to emulate eeprom
    let f = Flash::new_blocking(p.FLASH);
    let num_block = f.capacity() / Flash::<'_, Blocking>::ERASE_SIZE;
    let start = Flash::<'_, Blocking>::ERASE_SIZE * (num_block - 1);
    info!("FLASH write size, erase size {}, {}, {}", Flash::<'_, Blocking>::WRITE_SIZE, Flash::<'_, Blocking>::ERASE_SIZE, start);
